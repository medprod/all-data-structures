...
class LinkedList:
    ...
    def deleteEnd(self):
        if self.head is None:
            ...
            #traversing until we find second to last element 
            while self.head.next.next is not None:
                self.head = self.head.next
            #second to last element becomes last element
            self.head.next = None
           
    def deleteGiv(self, x):
        if self.head is None:
            print("Linked List is Empty")
            return
        #if given node is first node
        if x==self.head.data:
            self.head = self.head.next
            return
        while self.head.next is not None:
            #found previous node of given node
            if x==self.head.next.data:
                break
            #if x is not equal to given node, increment
            self.head = self.head.next
